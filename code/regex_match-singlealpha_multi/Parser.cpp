...
Parser::Parser(string in)
{
	...
	for (int i=0;i<input.length()-1;i++)
	{
		if (input[i] == '\\' &&(input[i+1]>=48&&input[i+1] <= 57))
		{
			...
			hasRefer = true;
			if (id2num.find(num) == id2num.end())
			{
				referID[groupCount] = num;
				id2num.insert(make_pair(num, -1));
				groupCount++;
			}
		}
...
shared_ptr<Regex> Parser::base()
{
	...
	case '\\':
	{
		...
		if (isdigit(esc))
		{
			...
			if (id > groupID)
				throw 3;
			int referno = referCount++;
			referNo2num.insert(make_pair(referno, id2num[id]));
			shared_ptr<Regex> re =make_shared<Refer>(referno);
