...
extern int wlen;
extern int lookaroundNum;
extern int referCount;
using namespace std;

#define colsize (wlen+3)
#define rawsize (referCount+2+lookaroundNum)
...
public:
	...
	//shared_ptr<unordered_set<pair<int, int>, Hashfunc>> cur_set;
	shared_ptr<boost::dynamic_bitset<>> cur_set;
	shared_ptr<boost::dynamic_bitset<>> next_set;
	//shared_ptr<unordered_set<pair<int, int>, Hashfunc>> next_set;
	int groupType;
	int groupNo;
	int c;
